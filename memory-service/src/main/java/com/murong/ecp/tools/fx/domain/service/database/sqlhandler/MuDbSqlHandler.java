...
@Slf4j
@Service("muDbHandler")
public class MuDbSqlHandler extends AbstractDataBaseHandler {
    ...
    public List<String> getTableColumns(Connection conn, String tableName, String schema) {
        ...
        // 使用SQL查询information_schema.columns获取列信息
        ...
        try (PreparedStatement stmt = conn.prepareStatement(sql)) {
            ...
            // 如果SQL查询失败，记录警告并尝试使用JDBC元数据API作为备选方案
            log.warn("使用SQL查询获取表 {} 列信息失败: {}, 尝试使用JDBC元数据API", tableName, e.getMessage());
            ...
            try (ResultSet rs = conn.getMetaData().getColumns(actualSchema, null, actualTableName, null)) {
                ...
                log.error("使用JDBC元数据API获取表 {} 列信息也失败: {}", tableName, metaEx.getMessage());
                throw metaEx;
            }
        }
        ...
        // 记录结果
        if (columns.isEmpty()) {
            log.warn("表 {} 没有找到列信息，schema: {}, 实际表名: {}", tableName, actualSchema, actualTableName);
        } else {
            log.info("表 {} 找到 {} 个列: {}", tableName, columns.size(), String.join(", ", columns));
        }
    ...
    public TableEntity getTableEntityFromDatabase(String tableName) {
        ...
        try (ResultSet rs = meta.getColumns(null, schema, tableName, null)) {
            while (rs.next()) {
                ...
                if(field.getLength()>9999){
                    if(!StringUtils.equals(field.getDbTyp(),"numeric")){
                        field.setDbTyp("text");
                        field.setLength(0);
                        field.setBizUpdSts(DataStatusEnum.UPD_DBTYP_LENGTH);
                    }else {
                        field.setLength(20);
                        field.setBizUpdSts(DataStatusEnum.UPD_LENGTH);
                    }
                }
                ...
                if(field.getLength()>0 &&StringUtils.equalsAny(field.getDbTyp(),"serial","float8","float4","int4","int8")){
                    field.setDbTyp("numeric");
                    field.setBizUpdSts(DataStatusEnum.UPD_DBTYP);
                }
                if(StringUtils.equalsAny(field.getDbTyp(),"bool","timestamp")){
                    field.setLength(0);
                    field.setBizUpdSts(DataStatusEnum.UPD_LENGTH);
                }
