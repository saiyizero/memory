package ${table.properties.mapperPath};
...
import ${table.properties.entityPath}.${table.tableNameCamel}PO;
...
public interface ${table.tableNameCamel}Mapper {
     ...
     * 根据主键查询
     */
    ${table.tableNameCamel}PO selectByPrimaryKey(<#list table.primaryKey.fields as pk>${table.rxFields?filter(f -> f.nameSnake == pk)[0].type} ${table.rxFields?filter(f -> f.nameSnake == pk)[0].nameCamel}<#if pk_has_next>, </#if></#list>);
     ...
     * 根据主键删除
     */
    int deleteByPrimaryKey(<#list table.primaryKey.fields as pk>${table.rxFields?filter(f -> f.nameSnake == pk)[0].type} ${table.rxFields?filter(f -> f.nameSnake == pk)[0].nameCamel}<#if pk_has_next>, </#if></#list>);
